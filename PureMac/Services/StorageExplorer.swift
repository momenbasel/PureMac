...
enum StorageExplorerError: LocalizedError, Equatable {
    ...
    var errorDescription: String? {
        ...
        case .notDirectory(let path):
            return String(format: String(localized: "The selected item is not a folder: %@"), path)
        case .symbolicLink(let path):
            return String(format: String(localized: "Symbolic links cannot be scanned: %@"), path)
        case .package(let path):
            return String(format: String(localized: "App and document packages are excluded: %@"), path)
        case .cloudOnly(let path):
            return String(format: String(localized: "The selected folder is stored in the cloud and has not been downloaded: %@"), path)
        case .unavailable(let path):
            return String(format: String(localized: "The selected folder could not be read: %@"), path)
        }
    }
}
